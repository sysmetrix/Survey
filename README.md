...
- **글꼴**: 보고서 화면의 *한글 문서 서식*에서 함초롬(기본)·공문서형(휴먼명조·HY헤드라인M)·맑은 고딕·나눔고딕·KoPub돋움체·Noto Sans KR·Pretendard GOV·직접 입력 중에서 고르고, 글자 크기(10~12pt)·줄 간격(150~180%)을 정합니다. 설정은 미리보기에 바로 반영되며 *이 PC에 글꼴이 있는지 확인*으로 설치 여부를 볼 수 있습니다.
...
- 새 버전이 배포되면 오른쪽 아래에 *지금 업데이트* 알림이 뜹니다. 버전 표시(`v5.3.0`)를 누르면 새 기능을 볼 수 있습니다.
...
- 버전을 올릴 때는 `js/main.js`의 `APP_VERSION`, `js/ui/history/version.js`, `package.json`, `index.html`의 `?v=`를 함께 바꾸고 `node tools/build-sw.mjs`를 실행합니다(테스트가 일치 여부를 검사).
