...
- **글꼴**: 보고서 화면의 *한글 문서 서식*에서 함초롬(기본)·공문서형(휴먼명조·HY헤드라인M)·맑은 고딕·나눔고딕·KoPub돋움체·Noto Sans KR·Pretendard GOV·직접 입력 중에서 고르고, 글자 크기(10~15pt)·줄 간격(150~180%)을 정합니다. 설정은 미리보기에 바로 반영되며 *이 PC에 글꼴이 있는지 확인*으로 설치 여부를 볼 수 있습니다. Windows·한글·글꼴 배포판에 따라 달라지는 패밀리명과 영문 설치명도 함께 확인합니다.
...
- 새 버전이 배포되면 오른쪽 아래에 *지금 업데이트* 알림이 뜹니다. 현재 버전은 상단에 `v5.3.3`으로 표시됩니다.
- 상단의 톱니바퀴에서 화면 테마, 자동 저장, 자동 버전 수와 보관 기간을 브라우저별로 설정할 수 있습니다. 화면이 이전 버전으로 보이면 *캐시 비우고 새로고침(Ctrl+F5)*으로 앱 파일만 다시 받으며 작업 내역과 설정은 유지됩니다.
