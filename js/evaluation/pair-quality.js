...
export function pairQuality(columns) {
  ...
  for (const items of groups.values()) {
    ...
    if (a[0].role !== b[0].role) add("role-mismatch", "사전·사후 문항의 분석 역할이 다릅니다.", items);
    ...
    if (a[0].role !== "numeric" && (a[0].scale?.min !== b[0].scale?.min || a[0].scale?.max !== b[0].scale?.max)) add("scale-mismatch", "사전·사후 척도 범위가 다릅니다.", items);
