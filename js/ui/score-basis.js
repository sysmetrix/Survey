...
let expanded = false; // 데이터 설정(compact) 화면에서 산식·예시를 펼쳐 보는 중인지
...
export function scoreBasisPanel(items = [], { compact = false } = {}) {
  ...
  const opt = (b, body) => `<label class="basis-opt${cur === b.id ? " on" : ""}">
      ...
      <div><b>${esc(b.label)}</b> <span class="badge info">${esc(b.tag)}</span>
        ${body ? `<p>${body}</p>` : ""}</div></label>`;
  const opts = (bodies) => `<div class="basis-opts" role="radiogroup" aria-label="100점 환산 기준">
      ${opt(SCORE_BASES[0], bodies?.[0])}
      ${opt(SCORE_BASES[1], bodies?.[1])}
    </div>`;
  const fullBody = () => `<p class="muted">환산 점수는 평균에서 계산합니다. 이때 평균을 <b>반올림하기 전 값</b>으로 할지, 보고서 표에 적히는 <b>반올림한 값(소수 둘째 자리)</b>으로 할지 고르세요. 이미 표의 평균으로 환산해 보고서를 써 오셨다면 ‘반올림 후’가 그 값과 일치합니다.</p>
    ...
    ${fileImpact}
    ${opts([
      "원자료의 평균 그대로 환산합니다. 가장 정확한 값입니다. 다만 표에 적힌 평균(소수 둘째 자리)으로 직접 계산하면 5점 척도 기준 최대 ±0.125점 차이가 날 수 있습니다.",
      "표에 적힌 평균으로 환산합니다. 표의 숫자로 직접 검산하면 그대로 맞고 기존 방식과 같습니다. 대신 반올림 오차(최대 ±0.125점)가 결과에 그대로 들어가, 판정 기준선(예: 목표 80점) 근처에서는 반올림 전 기준과 판정이 달라질 수 있고 평균이 같은 문항은 같은 점수로 정렬됩니다.",
    ])}
...
export const scoreBasisActions = {
  "basis-toggle": () => { expanded = !expanded; refresh(); },
