...
export function scoreBasisPanel(items = []) {
  ...
  const calc = (m, min, range, v) => `(${m} − ${min}) ÷ ${range} × 100 = <b>${f2(v)}점</b>`;
  const who = e => (e.label ? `「${esc(shortLabel(e.label))}」` : "평균이 다른 문항");
  const exactLines = examples.map((e, i) => `<li>예 ${i + 1}. ${who(e)} 평균 ${f4(e.ex.mean)} → ${calc(f4(e.ex.mean), e.ex.min, e.ex.max - e.ex.min, e.ex.exact)}</li>`).join("");
  const roundedLines = examples.map((e, i) => `<li>예 ${i + 1}. ${who(e)} 표에 ${f2(e.ex.shown)}로 적힘 → ${calc(f2(e.ex.shown), e.ex.min, e.ex.max - e.ex.min, e.ex.rounded)} <span class="muted">(${e.dir}, 반올림 전보다 ${signed(e.ex.rounded - e.ex.exact)}점)</span></li>`).join("");
  const fileImpact = imp.total
    ...
    : "";
  const opt = (b, body, lines) => `<label class="basis-opt${cur === b.id ? " on" : ""}">
      ...
      <div><b>${esc(b.label)}</b> <span class="badge info">${esc(b.tag)}</span>
        <p>${body}</p><ul class="basis-ex small muted">${lines}</ul></div></label>`;
  return `<section class="card basis-card" id="basis-card">
    ...
    <p class="basis-example">표에는 평균이 소수 둘째 자리로 반올림되어 적히므로, ‘반올림 후’ 기준의 환산 점수는 반올림 전보다 <b>높아질 수도(올림), 낮아질 수도(내림)</b> 있습니다.</p>
    ${fileImpact}
    <div class="basis-opts" role="radiogroup" aria-label="100점 환산 기준">
      ${opt(SCORE_BASES[0], "원자료의 평균 그대로 환산합니다. 가장 정확한 값입니다. 다만 표에 적힌 평균(소수 둘째 자리)으로 직접 계산하면 5점 척도 기준 최대 ±0.125점 차이가 날 수 있습니다.", exactLines)}
      ${opt(SCORE_BASES[1], "표에 적힌 평균으로 환산합니다. 표의 숫자로 직접 검산하면 그대로 맞고 기존 방식과 같습니다. 대신 반올림 오차(최대 ±0.125점)가 결과에 그대로 들어가, 판정 기준선(예: 목표 80점) 근처에서는 반올림 전 기준과 판정이 달라질 수 있고 평균이 같은 문항은 같은 점수로 정렬됩니다.", roundedLines)}
    </div>
