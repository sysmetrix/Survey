...
export function render() {
  ...
  const quick = quickKpis(r).map(q => guided ? { ...q, purpose: purposeOf(q), label: q.kpi.metric === "manual" ? q.label : `${METRICS[q.kpi.metric].label} · 측정 문항 선택 필요` } : q);
  ...
  <section class="card kpi-workspace">
    ...
    ${!guided || kpiTab === "quick" ? `<div class="kpi-tab-panel" data-panel="quick"><div class="row between wrap"><div><div class="eyebrow">빠른 설정</div><h2>성과지표 추가·확인</h2></div><div class="row gap">${state.businessFound?.kpi ? `<span class="badge ok">엑셀 성과지표 시트 반영됨</span>` : ""}<button class="btn sm" data-act="kpi-add">+ 빈 지표 추가</button></div></div>${guided ? `<div class="kpi-purpose-head"><div><h3>이번 사업에서 확인할 성과</h3><p class="small muted">목적을 선택하면 관련 지표를 강조합니다. 다른 지표도 그대로 비교·추가할 수 있습니다.</p></div><span class="badge ${selected.length ? "info" : "muted"}">${selected.length ? `${selected.length}개 선택` : "선택 전"}</span></div><div class="kpi-purpose-cards" role="group" aria-label="평가 목적 선택">${Object.entries(PURPOSES).map(([id,label]) => `<label class="kpi-purpose-card${selected.includes(id) ? " on" : ""}"><input type="checkbox" data-change="kpi-purpose" data-id="${id}" ${selected.includes(id) ? "checked" : ""}><span class="kpi-purpose-mark">${selected.includes(id) ? icon("check", 15) : icon("plus", 15)}</span><span><b>${esc(label)}</b><small>${id === "output" ? "운영 횟수·참여 인원" : id === "experience" ? "만족·참여 경험" : id === "change" ? "사전·사후 변화" : "활동 이후 실천"}</small></span></label>`).join("")}</div>${selected.includes("followup") ? '<p class="kpi-purpose-note">후속 실천은 활동 종료 뒤 별도 조사·관찰 기록이 필요합니다. 측정 시점과 실적 근거를 고급 설정에 기록하세요.</p>' : ""}<div class="kpi-recommend-head"><div><h3>추가할 지표</h3><p class="small muted">선택한 성과와 맞는 항목에는 추천 표시가 붙습니다.</p></div><span class="small muted">${quick.length}개 옵션</span></div>` : ""}<div class="quick-kpis${guided ? " kpi-recommendations" : ""}">${quick.map(qk => `<div class="quick-kpi-option${guided && selected.includes(qk.purpose) ? " recommended" : ""}">${guided && selected.includes(qk.purpose) ? '<span class="quick-kpi-recommended">선택한 성과에 추천</span>' : ""}<button class="chip-btn" data-act="kpi-quick" data-id="${qk.id}" ${used.has(qk.kpi.metric + "|" + qk.kpi.name) ? "disabled" : ""}>+ ${esc(qk.label)}</button><span class="quick-kpi-help">${esc(quickKpiHelp(qk))}</span></div>`).join("")}</div><datalist id="targetList">${targetOptions().map(t => `<option value="${esc(t)}">`).join("")}</datalist>${state.kpis.length ? guided ? kpiCards(state.kpis, r.evaluation?.results || [], targetChoices, { evidenceOn: isFeatureOn("referenceEvidence"), selectedId:selectedKpiId, filter:kpiFilter }) : kpiTable(r) : emptyKpi}${r.evaluation ? `<p class="summary">설정한 목표의 달성 요약: 판정 가능한 ${r.evaluation.summary.measured}개 중 <b>${r.evaluation.summary.achieved}개 달성</b>, ${r.evaluation.summary.mostly}개 대체로 달성, ${r.evaluation.summary.notAchieved}개 미달성 · 목표 없이 측정 ${r.evaluation.summary.unsetTarget}개</p>` : ""}${r.lint.length ? `<h3>연계 점검</h3><ul class="warnings">${r.lint.map(w => `<li>${levelBadge(w.level)} ${esc(w.msg)}</li>`).join("")}</ul>` : ""}</div>` : ""}
...
export const actions = {
  ...
  "kpi-select": el => { if (state.kpis.some(k => k.id === el.dataset.id)) selectedKpiId = el.dataset.id; refresh(); },
  "kpi-filter": el => { if (["all", "needs", "achieved", "mostly", "missed", "evidence"].includes(el.dataset.filter)) { kpiFilter = el.dataset.filter; selectedKpiId = ""; } refresh(); },
  "kpi-purpose": el => {
  ...
  kpi: el => {
    ...
    if (f === "id" && selectedKpiId === oldId) selectedKpiId = k.id;
    if (f === "metric" && !k.unit) k.unit = METRICS[k.metric]?.unit || "";
    invalidate(); refresh();
  },
  "kpi-add": () => { const k = { ...newKpi(nextKpiNo()), ...(isFeatureOn("guidedKpiSetup") ? { target:null,targetRef:"",requireTarget:true } : {}) }; state.kpis.push(k); selectedKpiId = k.id; kpiFilter = "all"; invalidate(); refresh(); },
  "kpi-quick": el => {
    const qk = quickKpis(compute()).find(x => x.id === el.dataset.id);
    if (!qk) return;
    const k = { ...newKpi(nextKpiNo()), ...qk.kpi, ...(isFeatureOn("guidedKpiSetup") ? { evaluationPurpose: (state.codebook.evaluationPurposes || []).join(","), name: qk.kpi.metric === "manual" ? qk.kpi.name : METRICS[qk.kpi.metric].label, target: null, targetRef: "", requireTarget: qk.kpi.metric !== "manual", stage: qk.kpi.metric === "manual" ? "산출" : "단기성과" } : {}) };
    state.kpis.push(k); selectedKpiId = k.id; kpiFilter = "all";
    invalidate();
