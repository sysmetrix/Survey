...
export function render() {
  ...
  const evidenceHtml = isFeatureOn("referenceEvidence") ? `<div class="reference-evidence"><div class="eyebrow">현재 프로젝트 적용 현황</div><h3>성과지표별 근거 검증 상태</h3>${state.kpis?.length ? `<div class="reference-evidence-list">${state.kpis.map(k => { const v = validateKpiEvidence(k, (METRICS[k.metric] || METRICS.manual).kind); return `<div class="reference-evidence-row"><b>${esc(k.name || k.id)}</b><span class="small ${v.valid ? "good-text" : "warn-text"}">${esc(v.valid ? `검토 완료 · ${v.evidence.title}` : v.reason)}</span></div>`; }).join("")}</div>` : `<p class="small muted">성과지표를 먼저 추가하면 지표별 근거 연결 상태가 표시됩니다.</p>`}</div>` : "";
