...
test("데이터 설정: 보기 점수 패널·일괄 적용", async () => {
  ...
  const quickHtml = setup.render();
  assert.ok(quickHtml.includes("빠른 설정") && quickHtml.includes("column-workspace"), "처음에는 빠른 설정의 문항 목록·선택 편집 패널을 표시");
  assert.ok(quickHtml.includes("확인 필요") && quickHtml.includes('data-mode="map"') && quickHtml.includes('data-mode="table"'), "초보자용 빠른 설정과 전체 표·분석 연결 보기의 목적을 함께 표시");
  assert.ok(quickHtml.includes("같이 묶어 볼 주제") && quickHtml.includes("한 문항만 분석하면 비워 두세요"), "문항 묶기 설정은 시스템 용어 대신 언제 쓰는지 설명한다");
  assert.ok(quickHtml.includes("분석 역할은 결과에 이 열을 쓰는 방법입니다"), "분석 역할의 의미를 선택 패널에서 바로 설명한다");
  ...
  assert.ok(html.includes("question-map") && html.includes("column-workspace") && html.includes("column-node") && html.includes("column-editor"), "분석 연결 보기는 노드와 선택 문항 편집 패널로 표시");
  assert.ok(html.includes("같이 묶어 볼 주제 (선택)"), "연결 구조 화면도 문항 묶기 용어를 빠른 설정과 통일");
  assert.ok(!html.includes('<table class="tbl setup">'), "분석 연결 보기에는 비교용 표를 중복 노출하지 않음");
  ...
  setup.actions["column-select"]({ dataset: { key: a.key } });
  setup.actions["setup-mode"]({ dataset: { mode: "quick" } });
  const tableHtml = setup.render();
  assert.ok(tableHtml.includes("빠른 설정") && tableHtml.includes("column-workspace"), "빠른 설정 탭은 가로 표 없이 행별 설정 기능을 유지");
  ...
  assert.ok(gridHtml.includes("전체 표 편집") && gridHtml.includes('class="tbl setup-grid"') && gridHtml.includes("현재 설정") && gridHtml.includes('class="setup-grid-detail"'), "숙련 사용자는 압축된 행을 비교하고 선택한 문항의 설정만 바로 아래에서 편집한다");
  assert.ok(gridHtml.includes('data-filter="review"') && gridHtml.includes("점수 방향 반대로") && gridHtml.includes("대표 만족도 문항"), "전체 표도 확인 필요 필터와 역문항·대표 만족 설정을 보존한다");
  setup.actions["setup-mode"]({ dataset: { mode: "map" } });
