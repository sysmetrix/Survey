...
test("데이터 설정: 보기 점수 패널·일괄 적용", async () => {
  ...
  assert.ok(quickHtml.includes("빠른 설정") && quickHtml.includes("bulk-question-card"), "처음에는 빠른 설정을 기본으로 표시");
  assert.ok(quickHtml.includes("확인 필요") && quickHtml.includes('data-mode="map"'), "처음 사용하는 사람에게 확인 순서와 연결 구조의 목적을 표시");
  assert.ok(quickHtml.includes("같이 묶어 볼 주제") && quickHtml.includes("한 문항만 분석하면 비워 두세요"), "문항 묶기 설정은 시스템 용어 대신 언제 쓰는지 설명한다");
  assert.ok(quickHtml.includes('class="bulk-question-options"'), "보조 설정은 기본 문항 행과 분리해 필요한 경우에만 연다");
  ...
  assert.ok(tableHtml.includes("빠른 설정") && tableHtml.includes("bulk-question-card"), "빠른 설정 탭은 가로 표 없이 행별 설정 기능을 유지");
